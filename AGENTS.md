...
A compiled static website documenting the chronology of **Gustavo Corção
(1896–1978)** — Brazilian Catholic writer, engineer and polemicist: convert
(1939), Centro Dom Vital intellectual, founder of Permanência (1968), columnist
of the major Rio papers through the military-regime years — an
intellectual-biography project in `olavo`'s pattern: life and works documented,
characterizations attributed, never adjudicated. A single JSON file is the
...
1. **There is NO documented Corção–TFP rupture — do not reintroduce one.**
   The sources establish 1950s adversarial relations, then convergence circa
   1969 (Jalles de Paula: "unindo-se a antigos adversários"). The relation is
   modeled as three attributed framings — the DHBB's TFP-linkage of
   Permanência, Permanência's own TFP-silent account contradicting it, and
   Jalles's former-adversaries-later-allies reading — and asserted as none of
   them. The documented ruptures are: Alceu Amoroso Lima / Centro Dom Vital
   (1963), the hierarchy (Dom Eugênio Sales, 1972–73), the Brazilian
   Benedictine Congregation (1974), and — per Jalles — Maritain in the
   Vatican II years. Any claim of a TFP rupture entering this dataset needs a
   primary document, dated and cited.
2. **The regime thread is carried as attributed framings, both directions.**
   Jalles's "apóstolo da linha-dura" / "um dos artífices… da legitimação"
   readings are his; the sympathetic profiles' omission of the thread is
   documented alongside. The site's own voice records the columns and their
   dates; it never adjudicates what they made him.
3. **Corpus quotes REQUIRE AUDIO.** COF-corpus reception entries are
   paraphrase with file id, date and review status (revisada /
   revisao_pendente); no verbatim quotation from a transcription until checked
   against audio. The "morreu doido" family claim is double-attributed (Olavo,
   in a 2015 lecture, reports the family is said to have claimed…) and
   asserted by no one.
4. **Permanência's founding dates are layered, not contradictory** — keep the
   August 1968 launch (press-dated) and the 29 September 1968 religious
   founding (Permanência's own date) both, each with its source. Do not state
   a first-issue date for the revista without a capture.
5. **Book years are discordant across the standard sources** — flag per work
   (the "Principal works" fact carries the discordances), never silently pick.
6. **Continuator and testimony sources are perspectives.** Permanência's
   pages are the continuators' self-description (labeled), the Fleichman
   interview is testimony (labeled), Centro Dom Bosco / Mosteiro da Santa
   Cruz / Krause are sympathetic (labeled). Single-source items from any of
   them stay flagged.
7. **Access ladder before citing** (register in `context.md`): scielo.br is
   UA-filtered; memoria.bn.gov.br is geo-blocked outside Brazil (the 1978
   obituaries and the 1968 O Globo launch report are attested via scholarship
   until captured); olavodecarvalho.org is hijacked — Wayback/print only;
   videeditorial.com.br is bot-walled. Read `KEYWORDS.md` before any corpus or
   web sweep — the manglings ("Corsão") and known-zero routes live there.
...
The five core rules of the `sourcing-rules` skill govern everything: cite it
or flag it; attribute, don't assert; sources span the spectrum by design;
time-sensitive statuses are dated; testimony and video are perspectives, not
fact sources. On top of them, this repo's contested terrain is listed in
"Subject-specific rules" above and in `context.md` ("The posture"); the
project's disambiguations live in the dataset's own `disambiguation.items[]`
(the TFP three-framings item is the load-bearing one). The thread lanes in
`meta.threads` are an editorial reading — five lanes, each with a cited
`basis`; note that no separate lane was given to "the TFP relation" precisely
because no rupture narrative exists to contain: the convergence events sit in
the church-disputes and permanencia lanes with the framings attributed in
their text.
