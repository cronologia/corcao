...
// This repo's dataset. The template shipped the example skeleton; corcao
// points the audit at the real data.
...
// reference ids here so the check below stays a real signal. In corcao every
// characterisation (perspective labels, hosting caveats, access notes) lives
// in publisherNote — which IS translated — so this set stays empty; add an id
// only for a bracket that NAMES the source, never for one that describes it.
